...
def main():
    ...
    # Проверка наличия файла наряда
    naryad_file = 'наряд-тестовый-_new_.csv'
    if not os.path.exists(naryad_file):
        print(f"Предупреждение: Файл наряда '{naryad_file}' не найден.")
        print("Некоторые новые функции не будут доступны.")
    else:
        print(f"✓ Файл наряда найден: {naryad_file}")
